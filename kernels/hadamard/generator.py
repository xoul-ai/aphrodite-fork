...
def string_to_array(string):
    ...
    return np.stack([np.fromstring(" ".join(string[i]),
                                   dtype=np.int32,
                                   sep=' ') for i in range(len(string))])
...
def array_code_gen(arr):
    ...
    for i in range(N):
        out.append(
            f"out[{i}] = " + " ".join(
                [f"{'+' if arr[i, j] == 1 else '-'} x[{j}]" for j in range(N)]
                ) + ";")
...
def main():
    ...
    output_dir.write_text(
        header + array_code_gen(string_to_array(had_12_paley)) +
