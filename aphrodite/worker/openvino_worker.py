...
from aphrodite.executor.openvino_executor import is_openvino_cpu
...
class OpenVINOWorker(LoraNotSupportedWorkerBase):
    ...
    def determine_num_available_blocks(self) -> Tuple[int, int]:
        ...
        kv_cache_space = envs.APHRODITE_OPENVINO_KVCACHE_SPACE
        cache_block_size = self.get_cache_block_size_bytes()
        kvcache_space_bytes = kv_cache_space * GiB_bytes
        ...
        if is_openvino_cpu():
            ...
            if kvcache_space_bytes > 0:
                logger.info("KV_CACHE size was explicitly configured via "
                            "APHRODITE_OPENVINO_KVCACHE_SPACE environment "
    ...
    def _init_cache_engine(self) -> None:
        ov_device = envs.APHRODITE_OPENVINO_DEVICE
        self.cache_engine = OpenVINOCacheEngine(
            self.cache_config,
            self.model_config,
    ...
    def profile_run(self) -> int:
        ...
        def model_profile_run():
            ...
            logger.info(
                f"Start profiling run with dummy inputs to evaluate "
                f"memory usage for {ov_device}. It might take a while.")
        ...
        if gpu_device_type == device.Type.INTEGRATED and \
            ...
            logger.warning(
                "iGPU is used with high "
                f"gpu_memory_utilization={memory_utilization} "
                ...
                "`APHRODITE_OPENVINO_KVCACHE_SPACE` (GB) environment "
                "variable.")
        ...
        logger.info(
            f"Total {ov_device} memory: {total_device_memory_str}. "
            "Amount of memory required to run the model with "
            f"max_num_batched_tokens={self.scheduler_config.max_num_batched_tokens}: "  # noqa: E501
            f"{used_device_memory_str}.")
