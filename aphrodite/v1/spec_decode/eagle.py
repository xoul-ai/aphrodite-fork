...
class EagleProposer:
    ...
    def __init__(
        ...
        self.method = self.aphrodite_config.speculative_config.method
        ...
        self.max_num_tokens = aphrodite_config.scheduler_config \
        ...
        self.arange = torch.arange(aphrodite_config.scheduler_config.max_num_seqs +
                                   1,
                                   device=device,
                                   dtype=torch.int32)
    ...
    def propose(
        ...
        # FIXME: The below two ops cause synchronization. Optimize.
        ...
        for _ in range(self.num_speculative_tokens - 1):
            ...
            # NOTE: We should handle the case where the draft model
    ...
    def load_model(self, target_model: nn.Module) -> None:
        loader = get_model_loader(self.aphrodite_config.load_config)
        target_layer_num = self.aphrodite_config.model_config.get_num_layers(
        ...
        target_device = self.aphrodite_config.device_config.device
        # We need to set the aphrodite_config here to register attention
        # layers in the forward context.
        with set_default_torch_dtype(
                draft_model_config.dtype), set_current_aphrodite_config(
                    self.aphrodite_config):
...
# NOTE: Currently, the below code is not used and we always use argmax
# to sample the draft tokens. We will use this after we find a way to manage
# the draft prob tensor.
# FIXME: The logic here is duplicated with the main sampling code.
...
def compute_probs_and_sample_next_token(
    ...
    # NOTE: Currently, we ignore most of the sampling parameters in
    ...
    q.exponential_()
    # NOTE: We shouldn't use `probs.div_(q)` because the draft_probs
