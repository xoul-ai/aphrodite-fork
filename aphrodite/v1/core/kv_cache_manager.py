...
class KVCacheManager:
    ...
    def __init__(
        ...
        # NOTE: To avoid frequent block allocation, we preallocate some
        # blocks for each request. For example, when a request reaches the end
        # of its block table, we preallocate N blocks in advance. This way, we
        # reduce the overhead of updating free_block_ids and ref_cnts for each
        # request every step (at the cost of some memory waste).
        # NOTE: This is different from the "lookahead" slots since this
    ...
    def get_computed_blocks(self, request: Request) -> List[int]:
        if not self.enable_caching:
            # No prefix caching.
            return []
        # TODO: Implement hash-based caching.
        return []
